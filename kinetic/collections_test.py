...
class TestIsValidKwargsDict(absltest.TestCase):
  def test_valid_identifiers(self):
    self.assertTrue(_is_valid_kwargs_dict({"lr": 0.1, "wd": 0.01}))

  def test_non_dict(self):
    self.assertFalse(_is_valid_kwargs_dict([1, 2, 3]))

  def test_non_string_keys(self):
    self.assertFalse(_is_valid_kwargs_dict({1: "a", 2: "b"}))

  def test_non_identifier_keys(self):
    self.assertFalse(_is_valid_kwargs_dict({"not-an-id": 1}))

  def test_keyword_keys(self):
    self.assertFalse(_is_valid_kwargs_dict({"class": 1, "name": 2}))

  def test_empty_dict(self):
    self.assertTrue(_is_valid_kwargs_dict({}))


class TestCallWithInput(absltest.TestCase):
  def test_auto_dict_valid_identifiers_unpacks_kwargs(self):
    fn = MagicMock(return_value="ok")
    _call_with_input(fn, {"a": 1, "b": 2}, "auto")
    fn.assert_called_once_with(a=1, b=2)

  def test_auto_dict_non_identifier_passes_as_single(self):
    fn = MagicMock(return_value="ok")
    d = {"not-id": 1}
    _call_with_input(fn, d, "auto")
    fn.assert_called_once_with(d)

  def test_auto_dict_with_keyword_passes_as_single(self):
    fn = MagicMock(return_value="ok")
    d = {"class": 1, "name": 2}
    _call_with_input(fn, d, "auto")
    fn.assert_called_once_with(d)

  def test_auto_list_unpacks_args(self):
    fn = MagicMock(return_value="ok")
    _call_with_input(fn, [1, 2, 3], "auto")
    fn.assert_called_once_with(1, 2, 3)

  def test_auto_tuple_unpacks_args(self):
    fn = MagicMock(return_value="ok")
    _call_with_input(fn, (10, 20), "auto")
    fn.assert_called_once_with(10, 20)

  def test_auto_scalar_passes_single(self):
    fn = MagicMock(return_value="ok")
    _call_with_input(fn, 42, "auto")
    fn.assert_called_once_with(42)

  def test_single_mode(self):
    fn = MagicMock(return_value="ok")
    _call_with_input(fn, [1, 2], "single")
    fn.assert_called_once_with([1, 2])

  def test_args_mode_spreads_list(self):
    fn = MagicMock(return_value="ok")
    _call_with_input(fn, [10, 20], "args")
    fn.assert_called_once_with(10, 20)

  def test_args_mode_rejects_non_sequence(self):
    fn = MagicMock()
    with self.assertRaises(TypeError):
      _call_with_input(fn, 42, "args")

  def test_kwargs_mode_spreads_dict(self):
    fn = MagicMock(return_value="ok")
    _call_with_input(fn, {"x": 1}, "kwargs")
    fn.assert_called_once_with(x=1)

  def test_kwargs_mode_rejects_non_dict(self):
    fn = MagicMock()
    with self.assertRaises(TypeError):
      _call_with_input(fn, [1, 2], "kwargs")

  def test_unknown_mode_raises(self):
    fn = MagicMock()
    with self.assertRaises(ValueError):
      _call_with_input(fn, 1, "bogus")
...
class TestManifestHelpers(absltest.TestCase):
  ...
  def test_build_initial_manifest_defaults(self):
    m = _build_initial_manifest("grp-2", "map", None, None, 5, "fn")
    self.assertIsNone(m["group_name"])
    self.assertEqual(m["tags"], {})

  def test_append_child_new(self):
    m = {"children": []}
    _append_child_to_manifest(m, 0, "job-a")
  ...
  def test_append_child_updates_existing(self):
    ...
    _append_child_to_manifest(m, 0, "job-new", attempts=2)
    self.assertEqual(len(m["children"]), 1)
...
class TestMap(absltest.TestCase):
  ...
  def test_manifest_updated_per_job(self):
    submit_fn = self._make_submit_fn()
    with (
      mock.patch(
        "kinetic.collections.storage.upload_manifest"
    ...
    # 1 initial + 3 per-job updates
    self.assertEqual(mock_manifest.call_count, 4)

  def test_input_mode_auto_with_dict(self):
    fn = MagicMock(return_value=_make_handle())
    fn.__name__ = "fn"
    with (
      mock.patch("kinetic.collections.storage.upload_manifest"),
      mock.patch("kinetic.collections.storage.upload_handle"),
  ...
  def test_warn_large_unbounded(self):
    submit_fn = self._make_submit_fn()
    with (
      mock.patch("kinetic.collections.storage.upload_manifest"),
      mock.patch("kinetic.collections.storage.upload_handle"),
      mock.patch("kinetic.collections.logging.warning") as mock_warn,
    ):
      map(
        submit_fn,
        list(range(101)),
    ...
    # Find the warning about large unbounded submission.
    warned = any(
      "max_concurrent=None" in str(c) for c in mock_warn.call_args_list
    )
...
class TestMapFailFast(absltest.TestCase):
...
class TestMapWithRetries(absltest.TestCase):
...
class TestAttachBatch(absltest.TestCase):
  ...
  def test_partial_submission_warns(self):
    manifest = self._make_manifest(n_children=2, total_expected=5)
    ...
    with (
      ...
      attach_batch("grp-abc12345", project="proj", cluster="cluster")

    warned = any(
      "partially submitted" in str(c) for c in mock_warn.call_args_list
    )
    self.assertTrue(warned)
  ...
  def test_partial_submission_has_correct_length(self):
    """Partial submission should preallocate to total_expected."""
    manifest = self._make_manifest(n_children=2, total_expected=5)
    ...
    # 5 slots preallocated, only 2 populated.
    self.assertEqual(len(handle.jobs), 5)


# ------------------------------------------------------------------
# Submission errors surfaced in results()
# ------------------------------------------------------------------
...
class TestSubmissionErrors(absltest.TestCase):
  ...
  def test_submission_error_stored_on_handle(self):
    """The _submission_errors dict should record per-input failures."""
    call_count = [0]
    ...
    self.assertIn(0, handle._submission_errors)
    self.assertIsInstance(handle._submission_errors[0], ValueError)


# ------------------------------------------------------------------
# results() with ordered=False
# ------------------------------------------------------------------


class TestResultsUnordered(absltest.TestCase):
...
# ------------------------------------------------------------------
# Non-daemon submission thread (P1 fix)
# ------------------------------------------------------------------


class TestNonDaemonThread(absltest.TestCase):
  def test_submission_thread_is_not_daemon(self):
    ...
    submit_fn = lambda x: _make_handle(job_id=f"job-{x}")
    submit_fn.__name__ = "fn"
    ...
    # There should be exactly one thread started (the submission thread).
    self.assertEqual(len(started_threads), 1)
    self.assertFalse(started_threads[0].daemon)


# ------------------------------------------------------------------
# Fail-fast observes runtime failures after all jobs submitted (P2 fix)
# ------------------------------------------------------------------


class TestFailFastObservesRuntimeFailures(absltest.TestCase):
  def test_cancel_siblings_on_runtime_failure(self):
    ...
    # Simulate: job-0 transitions RUNNING->FAILED. After cancel is
    # called on job-1, it transitions to NOT_FOUND (terminal).
    cancelled = set()
    poll_counts = {}
    ...
    def mock_status(self_handle):
      ...
      if jid == "job-0":
        if poll_counts[jid] >= 2:
          return JobStatus.FAILED
        return JobStatus.RUNNING
      # job-1: terminal once cancelled, running otherwise.
      if jid in cancelled:
    ...
    # job-1 should have been cancelled when job-0 failed.
    self.assertIn("job-1", cancelled)


# ------------------------------------------------------------------
# as_completed() streams before submission finishes (P2 fix)
# ------------------------------------------------------------------


class TestAsCompletedStreaming(absltest.TestCase):
  def test_yields_before_submission_complete(self):
...
# ------------------------------------------------------------------
# Unordered results() honors timeout (P2 fix)
# ------------------------------------------------------------------


class TestUnorderedResultsTimeout(absltest.TestCase):
  def test_unordered_results_timeout(self):
    """results(ordered=False, timeout=...) must raise TimeoutError
    when a job never completes, not hang forever."""
    handle = _make_batch_handle(n_jobs=1, submission_complete=True)
    ...
    with (
      ...
      handle.results(ordered=False, timeout=5)


# ------------------------------------------------------------------
# max_concurrent validation (P2 fix)
# ------------------------------------------------------------------


class TestMaxConcurrentValidation(absltest.TestCase):
  ...
  def test_negative_max_concurrent_rejected(self):
    with self.assertRaisesRegex(ValueError, "max_concurrent"):
      map(lambda x: x, [1], max_concurrent=-1)

  def test_positive_max_concurrent_accepted(self):
    submit_fn = lambda x: _make_handle(job_id="job-0")
    submit_fn.__name__ = "fn"
  ...
  def test_none_max_concurrent_accepted(self):
    """max_concurrent=None (unlimited) should still work.
    The simple path (no concurrency limit, no retries) submits all
    jobs synchronously without polling status."""
    submit_fn = lambda x: _make_handle(job_id="job-0")
    submit_fn.__name__ = "fn"
...
# ------------------------------------------------------------------
# Bounded concurrency actually limits active jobs (integration-style)
# ------------------------------------------------------------------


class TestBoundedConcurrencyIntegration(absltest.TestCase):
  def test_max_concurrent_limits_active_jobs(self):
    """With max_concurrent=2, at most 2 jobs should be active at any
    point during the submission loop. This tests real concurrency
    behavior — the submit_fn records concurrency at call time."""
    max_active_seen = [0]
    currently_active = [0]
    lock = threading.Lock()
    ...
    # Each job transitions RUNNING -> SUCCEEDED after 2 polls.
    # Track which jobs have already been decremented to avoid
    # double-counting.
    poll_counts = {}
    decremented = set()
    ...
    # All 5 jobs should have been submitted.
    self.assertEqual(len([j for j in handle.jobs if j is not None]), 5)
    # At no point should more than 2 have been active concurrently.
