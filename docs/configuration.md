...
State backend   | _(n/a)_            | `--state-backend`                | `KINETIC_STATE_BACKEND`                         | `state_backend` (then settings)      | `local` (`file://~/.kinetic/pulumi`)
...
from (env var, [profile](guides/profiles.md), global setting, or
